...
public static class SetupReviewSummaryBuilder
{
    public static SetupReviewSummary Build(SetupConfig config, string? dataDir = null, string? localDataDir = null)
    {
        ...
        var runtimeArgument = isCustomInstaller
            ? ""
            : $" --node-version {GatewayInstallPolicy.NodeVersion}";
        var versionArgument = string.IsNullOrWhiteSpace(requestedVersion)
            ? ""
            : $" --version {requestedVersion}";
        var installCommand =
            $"curl -fsSL --proto '=https' --tlsv1.2 <install-url> | bash -s --{versionArgument}{runtimeArgument}";
