'use strict';
// Private request-owned WSL supervisor. No public endpoint or user configuration.
...
async function broker(s, source) {
  ...
  const unit='openclaw-browser-bootstrap-'+s.requestId+'.service';
  const description='OpenClaw browser bootstrap '+s.requestId;
  const uid=process.getuid(),runtime='/run/user/'+uid;
  const state={...s,mode:'gate',unit,description};
  const program='const settings='+JSON.stringify(state)+';const supervisorSource='+JSON.stringify(source)+';'+Buffer.from(source,'base64').toString();
  ...
  new Reader(runner.stdout,value=>{
    if(value.type==='ready') {
      ...
      ready=value;
      // Even an already-revoked request needs this verified tuple to authenticate SETTLED.
      // Forwarding READY grants no permission: the closed input can never deliver PERMIT.
      emit(ready);
      if(cancelled)stop();
      return;
    }
...
if(typeof settings!=='undefined') {
  ...
  (settings.mode==='gate'?gate(settings):broker(settings,supervisorSource)).catch(()=>{process.stderr.write('pairing_unavailable\n');process.exit(46);});
}
