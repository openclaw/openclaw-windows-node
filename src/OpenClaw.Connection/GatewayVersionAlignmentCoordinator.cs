...
public sealed partial class GatewayVersionAlignmentCoordinator
{
        ...
        Func<GatewayUpdateProtectionMode>? protectionModeResolver = null)
    {
        ...
        _synchronizeAsync = synchronizeAsync ?? ((_, _) =>
            throw new InvalidOperationException(
                "Gateway synchronization is required before an in-place update."));
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (before.State == GatewayVersionAlignmentState.Aligned && pending is not null)
            {
                ...
                if (pendingState != GatewayUpdateDispatchState.Accepted ||
                    (pendingRoute == GatewayPackageUpdateRoute.CoreTransaction && pendingTransaction is null))
                {
                    var dispatchFailure = pendingRoute == GatewayPackageUpdateRoute.CoreTransaction
                        ...
                        : "The aligned Companion installer receipt has no accepted dispatch provenance. A second installer dispatch was blocked because the earlier result may be ambiguous; explicit recovery is required.";
                    return Result(
                        GatewayVersionAlignmentState.RecoveryAvailable,
                        before.InstalledVersion,
                        pending.OpenClawVersion,
        ...
        CancellationToken cancellationToken)
    {
        ...
        var pending = _rollbackPoints.FindPendingUpdates().SingleOrDefault(candidate =>
            string.Equals(candidate.Id, pointId, StringComparison.Ordinal) &&
            string.Equals(candidate.GatewayId, expectedGatewayId, StringComparison.Ordinal));
        if (pending is null)
