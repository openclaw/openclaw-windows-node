...
internal sealed class TrayMenuPresenter
{
    ...
    internal TrayMenuPresentation Present()
    {
        ...
        var foregroundSessions = _snapshot.Sessions
            .Where(session => !IsBackground(session))
            .ToArray();
    ...
    {
        var active = sessions.Count(session => string.Equals(session.Status, "active", StringComparison.OrdinalIgnoreCase));
        ...
        return new TrayMenuElement
        {
            ...
            Text = "Sessions",
            Detail = $"{active} active · {FormatTokenCount(totalTokens)} tokens",
            ActionId = "sessions",
            AutomationName = $"Sessions. {active} active. {FormatTokenCount(totalTokens)} tokens.",
            Children = children.ToImmutable(),
        };
    }
    ...
    private static bool IsBackground(TraySessionSnapshot session) =>
        SessionPresentationResolver.IsBackground(session.ToSessionInfo());
