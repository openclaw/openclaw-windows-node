...
internal static class ChatEventMapper
{
    ...
    private static ChatEvent? MapTool(AgentEventInfo evt)
    {
        ...
        string? toolCallId = NativeToolProjector.GetStringProperty(
            evt.Data,
            "itemId",
            "callId");
        ...
        return phase.ToLowerInvariant() switch
        {
            ...
            "result" => new ChatToolOutputEvent(
                NativeToolProjector.ExtractToolResultText(evt.Data, label),
                ToolCallId: toolCallId,
                RunId: evt.RunId),
    ...
    private static ChatEvent? MapItem(AgentEventInfo evt)
    {
        ...
        if (string.Equals(kind, "command", StringComparison.OrdinalIgnoreCase))
        {
            ...
            var childItemId = NativeToolProjector.GetStringProperty(
                evt.Data,
                "itemId",
                "commandItemId",
                "callId");
        ...
        var itemId = NativeToolProjector.GetStringProperty(
            evt.Data,
            "itemId",
            "callId");
    ...
    private static ChatEvent? MapCommandOutput(AgentEventInfo evt)
    {
        ...
        if (string.IsNullOrEmpty(output))
            return null;
        string? itemId = NativeToolProjector.GetStringProperty(
            evt.Data,
            "parentItemId",
            "itemId");
        if (string.IsNullOrWhiteSpace(itemId))
