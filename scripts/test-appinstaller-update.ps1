<#
.SYNOPSIS
  Simulates a non-Store .appinstaller upgrade by hosting two signed MSIX
  versions on a local HTTP server and walking the install vN -> publish vN+1
  -> trigger upgrade flow end-to-end.

.DESCRIPTION
  The point of this script is to catch regressions in the .appinstaller XML
  and the PackageManager.AddPackageByAppInstallerFileAsync wiring without
  needing a real GitHub release / stable-feed PR cycle. Run before a release
  tag goes out; if it fails, the same failure will reach every user who
  installs from the stable architecture-specific AppInstaller URL.

  Steps:
   1. Launch a tiny HTTP server (HttpListener) on localhost:$Port that serves
      the two MSIX files plus a rendered .appinstaller pointing at vN+1.
   2. Render an "old" .appinstaller pointing at vN and install it (records
   ...
   4. Invoke PackageManager.AddPackageByAppInstallerFileAsync against the
      local URL — same call the OS performs in the AutomaticBackgroundTask.
   5. Assert Get-AppxPackage reports the new Version.
   6. Tear down.

  Requires signed MSIX packages — Add-AppxPackage / AddPackageByAppInstaller
  refuse unsigned packages outside a developer-mode loopback. Use the dev
  signing cert from scripts/setup-dev-msix-cert.ps1 to produce vN and vN+1.

.PARAMETER MsixVnPath
  Path to the "older" signed .msix (used as the seed install).

.PARAMETER MsixVn1Path
  Path to the "newer" signed .msix (used as the upgrade target).
...
param(
  ...
  [string] $Publisher = 'CN=OpenClaw Foundation, O=OpenClaw Foundation, L=Mill Valley, S=California, C=US',
  [int]    $Port = 8765
)
...
try {
  ...
  # prefix in both parent and job would fail before AppInstaller is exercised.
  ...
  try {
    ...
    # Step 4: trigger the upgrade via PackageManager (same call the OS uses).
