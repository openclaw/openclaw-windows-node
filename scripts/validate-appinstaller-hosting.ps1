<#
.SYNOPSIS
  Validates a hosted AppInstaller XML and its referenced MSIX URL before
  promoting a release.

.DESCRIPTION
  Windows AppInstaller is strict about hosted metadata and package assets.
  This script checks the stable .appinstaller URL (or a local file), parses
  its MainPackage URI when -MsixUri is not provided, then validates the MSIX
  endpoint for content-type, content-length, and range-request support.

  Intended for release operators before promoting
  installer/appinstaller/openclaw-{x64,arm64}.appinstaller to the stable
  feed location, and for the appinstaller-feed-pr workflow.
...
.PARAMETER MsixUri
  ...
  reads AppInstallerPath) and uses the MainPackage Uri attribute.
...
.PARAMETER AllowGitHubContentTypes
  ...
  downloads serve MSIX files as application/octet-stream and raw .appinstaller
  files as text/plain. This switch keeps strict validation as the default
  while allowing the GitHub-hosted production flow.
...
function Assert-ContentType {
  ...
  if ([string]::IsNullOrWhiteSpace($contentType) -or $allowed.Count -eq 1) {
...
function Assert-ContentLength {
  ...
  $parsed = 0L
  if (-not [long]::TryParse($contentLength, [ref]$parsed)) {
