...
public sealed class BrowserWslProductionOwnerTests
{
    ...
    private async Task RunCase(E2ESetupFixture fixture,string name)
    {
        ...
        JsonElement unit=default;bool unknown=false;
        try
        {
            ...
            var retirement=ownership.Retire();
            ...
            {
                var pid=FindProductionClient(fixture.DistroName);
                using var process=Process.GetProcessById(pid);process.Kill();await process.WaitForExitAsync();
                await retirement.WaitAsync(TimeSpan.FromSeconds(40));
            ...
            var empty=await WaitUnit(fixture,p=>!p.GetProperty("present").GetBoolean()||p.GetProperty("processes").GetArrayLength()==0,TimeSpan.FromSeconds(5),unitName);
            _cases.Add(new{name,identities=ids,commandEndTicks=commandEnd,clockFrequency=Stopwatch.Frequency,outcome,
                ...
                actualOwnerExecuted=true,guestAcknowledgmentRequiredByOwner=!unknown,unknownRetainsActivation=unknown,resultReturned=pairing is not null,
    ...
    private static int FindProductionClient(string distro)
    {
        var ps=Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.System),"WindowsPowerShell","v1.0","powershell.exe");
        var script="$p=@(Get-CimInstance Win32_Process -Filter \"Name='wsl.exe'\" | Where-Object {$_.CommandLine -like '*--distribution "+distro+"*--exec /bin/bash --noprofile --norc -s*'});if($p.Count-ne 1){throw 'owner client identity'};[Console]::Out.Write($p[0].ProcessId)";
        ...
        using var process=Process.Start(start)!;var output=process.StandardOutput.ReadToEnd();process.WaitForExit(5000);Assert.Equal(0,process.ExitCode);return int.Parse(output.Trim());
    }
