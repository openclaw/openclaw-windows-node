...
public sealed class GatewayVersionAlignmentCoordinatorTests : IDisposable
{
    ...
    [Fact]
    public async Task UpdateAsync_AlignsNewerUnauditedVersionThroughVerifiedInstallerAndHealthGate()
    {
        var runner = new FakeWslCommandRunner();
            ...
            (_, _) =>
            {
                ...
                if (synchronizations == 1)
                {
                    Assert.DoesNotContain(
                        runner.Calls,
                        call => call.Kind == "direct" &&
                                call.Arguments.FirstOrDefault() == "--export");
                    Assert.DoesNotContain(
                        runner.Calls,
        ...
        string firstAdoptionVersion)
    {
        ...
        var coordinator = new GatewayVersionAlignmentCoordinator(
            ...
            synchronizeAsync: (_, _) => Task.CompletedTask,
    ...
    [Fact]
    public async Task FindPendingUpdate_FiltersByGatewayIdentity()
    {
        var runner = new FakeWslCommandRunner();
        var manager = CreateManager(runner);
        EnqueueCreateAttestation(runner, PreviousVersion);
        var created = await manager.CreateVerifiedAsync(
            "OpenClawGateway", "original-gateway", PreviousVersion, RequiredVersion);
        manager.MarkUpdateInProgress(created.Point!.Id);

        Assert.Null(manager.FindPendingUpdate("replacement-gateway", RequiredVersion));
        Assert.Equal(
            created.Point.Id,
            manager.FindPendingUpdate("original-gateway", RequiredVersion)?.Id);
    }

    [Fact]
    public async Task VerifyAsync_RejectsManifestWhoseIdDoesNotMatchRequestedPoint()
    {
        var runner = new FakeWslCommandRunner();
        var manager = CreateManager(runner);
        EnqueueCreateAttestation(runner, PreviousVersion);
        var created = await manager.CreateVerifiedAsync(
            "OpenClawGateway", "local-gateway", PreviousVersion, RequiredVersion);
        var originalId = created.Point!.Id;
        ...
        var swappedId = $"20260720T120000000Z-{Guid.NewGuid():N}";
        manifestJson["Id"] = swappedId;
        File.WriteAllText(manifestPath, manifestJson.ToJsonString());

        Assert.False(await manager.VerifyAsync(originalId));
    ...
    [ReparsePointFact]
    public async Task MarkUpdateInProgress_RejectsReparseBackedPointDirectory()
    {
        var runner = new FakeWslCommandRunner();
        var manager = CreateManager(runner);
        EnqueueCreateAttestation(runner, PreviousVersion);
        ...
        var pointDirectory = Path.Combine(
            ...
            "OpenClawGateway",
            created.Point!.Id);
        var redirected = Path.Combine(_tempRoot, "redirected-existing-point");
        Directory.Move(pointDirectory, redirected);
        Directory.CreateSymbolicLink(pointDirectory, redirected);

        Assert.Throws<IOException>(() => manager.MarkUpdateInProgress(created.Point.Id));
        Assert.Equal(
            GatewayRollbackPointPhase.Verified,
            Enum.Parse<GatewayRollbackPointPhase>(
                JsonNode.Parse(File.ReadAllText(Path.Combine(redirected, "manifest.json")))!
                    ["Phase"]!
                    .GetValue<string>()));
    }
    ...
    [InlineData("2026.7.23")]
    public async Task UpdateAsync_FailsClosedBeforeProtectionWhenPreUpdateHealthFailsForEitherRoute(
        string installedVersion)
    {
        var runner = new FakeWslCommandRunner();
        runner.EnqueueDistro(Ok(installedVersion));
        var synchronizations = 0;
        var coordinator = CreateCoordinator(
            runner,
            (_, _) =>
            {
                synchronizations++;
                return Task.FromException(new InvalidOperationException("private detail"));
            });
        ...
        Assert.Empty(coordinator.ListRollbackPoints());
        Assert.DoesNotContain(
            runner.Calls,
            call => call.Kind == "direct" &&
                    call.Arguments.FirstOrDefault() == "--export");
    ...
    [Fact]
    public async Task UpdateAsync_FailsClosedBeforeProtectionWhenSynchronizationDependencyIsMissing()
    {
        var runner = new FakeWslCommandRunner();
        runner.EnqueueDistro(Ok("2026.7.23"));
        var coordinator = new GatewayVersionAlignmentCoordinator(
    ...
    [Fact]
    public async Task UpdateAsync_AlignedUnacceptedInstallerRequiresExplicitRecovery()
    {
        var runner = new FakeWslCommandRunner();
        var manager = CreateManager(runner);
        ...
        Assert.Equal(GatewayVersionAlignmentState.RecoveryAvailable, result.State);
        Assert.Contains("no accepted dispatch provenance", result.FailureSummary, StringComparison.Ordinal);
        Assert.Equal(0, synchronizations);
        var pending = Assert.Single(manager.FindPendingUpdates());
        Assert.Equal(GatewayUpdateDispatchState.Prepared, pending.UpdateDispatchState);
    }

    [Fact]
    public async Task ResolveNativeRecovery_CompletesHistoricalCoreReceiptByPointIdentity()
    {
        ...
        var coordinator = new GatewayVersionAlignmentCoordinator(
            ...
            synchronizeAsync: (_, _) => Task.CompletedTask,
            gatewayRequestAsync: updateRpc.SendAsync,
            connectedGatewayId: () => "historical-gateway",
            routePolicy: new GatewayPackageUpdateRoutePolicy([PreviousVersion]));
        ...
        var completion = Assert.Single(updateRpc.Calls, call => call.Method == "update.complete");
        Assert.Equal("historical-gateway", completion.ExpectedGatewayId);
        Assert.Equal("historical-transaction", completion.Parameters.GetProperty("transactionId").GetString());
    ...
    [Theory]
    [InlineData(GatewayUpdateProtectionMode.FullVhd, "rollback.vhdx", true)]
    [InlineData(GatewayUpdateProtectionMode.NativeBackup, "openclaw-backup.tar.gz", false)]
    public async Task CreateVerifiedAsync_PreservesVerifiedArtifactWhenManifestCommitIsAmbiguous(
        ...
        bool restoreEligible)
    {
        ...
        var manager = CreateManager(
            runner,
            afterManifestCommit: manifest =>
            {
                if (!injected &&
                    manifest.VerificationStatus == GatewayRollbackPointVerificationStatus.Verified &&
                    manifest.LastFailureCode is null)
                {
                    injected = true;
                    throw new IOException("Simulated post-commit flush failure.");
                }
            });
        ...
        Assert.Equal(GatewayRollbackOperationState.Failed, result.State);
        var point = Assert.Single(manager.List());
        Assert.Equal(GatewayRollbackPointVerificationStatus.Verified, point.VerificationStatus);
        Assert.Equal(restoreEligible, point.RestoreEligible);
    ...
    [Fact]
    public async Task Cleanup_PreservesManifestUntilLockedPayloadCanBeRemoved()
    {
        if (!OperatingSystem.IsWindows())
            return;

        var current = new DateTimeOffset(2026, 7, 20, 12, 0, 0, TimeSpan.Zero);
        var runner = new FakeWslCommandRunner();
        var manager = CreateManager(runner, () => current);
        var old = await CreateHealthyPointAsync(manager, runner, "2026.6.1");
        current += TimeSpan.FromDays(1);
        await CreateHealthyPointAsync(manager, runner, "2026.7.1");
        var oldDirectory = Path.Combine(
            _tempRoot,
            "gateway-rollback-points",
            "OpenClawGateway",
            old.Id);
        var manifestPath = Path.Combine(oldDirectory, "manifest.json");
        var vhdPath = Path.Combine(oldDirectory, "rollback.vhdx");

        await using (File.Open(vhdPath, FileMode.Open, FileAccess.Read, FileShare.None))
        {
            Assert.Equal(0, await manager.CleanupAsync(new(1, null)));
            Assert.True(File.Exists(manifestPath));
            Assert.False(manager.HasUnreadableReceipt());
        }
    ...
    [Fact]
    public async Task Cleanup_UpdatesFailedPointStorageAfterRemovingInvalidArtifact()
    {
        var runner = new FakeWslCommandRunner();
        var manager = CreateManager(runner);
        EnqueueCreateAttestation(runner, PreviousVersion);
        ...
        var failed = Assert.Single(manager.List());
        Assert.Equal(created.Point.Id, failed.Id);
        Assert.Equal(0, failed.ApproximateSizeBytes);
    }
    ...
    {
        synchronize ??= (_, _) => Task.CompletedTask;
    ...
    private GatewayRollbackPointManager CreateManager(
        ...
        Action<GatewayRollbackPointManifest>? afterManifestCommit = null)
    {
