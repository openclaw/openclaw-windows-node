...
public class SessionInfoContextSummaryTests
{
    ...
    [Fact]
    public void DangerousCommands_IncludesOllamaChat()
    {
        // ollama.chat is privacy/resource-sensitive (sends prompt content to a
        // locally running model and consumes CPU/GPU), same tier as tts.speak
        // and stt.transcribe.
        Assert.Contains("ollama.chat", CommandCenterCommandGroups.DangerousCommands);
        Assert.Contains("ollama.chat", (IReadOnlySet<string>)CommandCenterCommandGroups.DangerousCommandSet);
    }

    [Fact]
    public void MacNodeParityCommands_ExcludesOllamaCommands()
    {
        // Ollama availability is optional on every platform (it depends on a
        // separately installed Windows service), so neither ollama.* command
        // should ever be flagged as "missing" Mac parity.
    ...
    [Fact]
    public void OptionalSafeCommands_IncludesOllamaModels()
    {
        // ollama.models is read-only inventory - safe like the
        // SafeCompanionCommands - but it must not be spliced into
        // MacNodeParityCommands, so it lives in its own optional set.
        Assert.Contains("ollama.models", CommandCenterCommandGroups.OptionalSafeCommands);
        Assert.Contains("ollama.models", (IReadOnlySet<string>)CommandCenterCommandGroups.OptionalSafeCommandSet);
    ...
    [Fact]
    public void FromNode_OllamaModelsAllowed_AppearsInSafeApprovedCommands()
    {
        var node = new GatewayNodeInfo
        {
            NodeId = "node-1",
    ...
    [Fact]
    public void FromNode_OllamaModelsPermissionBlocked_AppearsInMissingSafeAllowlistCommands()
    {
        var node = new GatewayNodeInfo
        {
            NodeId = "node-1",
        ...
        // ollama.models is safe/read-only, so a missing allowlist entry
        // gets the same "safe command" repair guidance as SafeCompanionCommands.
