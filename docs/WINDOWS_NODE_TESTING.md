...
| `tts.speak` | Speak text aloud | Requires Text-to-speech playback enabled in Settings; gateway mode also requires `tts.speak` in the version-appropriate allowlist (`gateway.nodes.commands.allow` current/frozen; `gateway.nodes.allowCommands` on `2026.6.11` and `2026.7.2-beta.3`) |
