...
Native mode downloads a pinned Node runtime from nodejs.org, verifies its SHA-256, and installs both Node and the pinned OpenClaw CLI inside an app-owned LocalAppData prefix. It configures an isolated `OpenClawGateway` profile and hardens its per-user Windows Scheduled Task with bounded restart-on-failure. Existing system Node installations, global OpenClaw packages, wrappers, PATH entries, and the default profile are preserved. WSL mode installs and connects a new app-owned `OpenClawGateway` instance from a clean WSL baseline. It does not export from or mutate an existing user Ubuntu distro; if WSL cannot create the named app-owned distro directly, setup fails with an actionable update message.
