...
| dashboard-link-policy | authoritative | src/OpenClaw.Tray.WinUI/App.xaml.cs + App.CapabilityHandlers.cs | duplicated Tailscale revalidation, browser-credential fallback, and dashboard URL policy | GatewayDashboardLinkService | App retains UI launch/error side effects; local MCP retains response projection | trusted Tailscale links omit shared credentials; failed revalidation falls back only to an already approved browser credential and otherwise fails closed | AppRefactorContractTests.DashboardLinkPolicy_StaysDelegatedToFocusedService | source-shape | when App and the local MCP no longer own dashboard-link entrypoints |
