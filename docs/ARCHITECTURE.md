...
| `src/OpenClaw.Connection/GatewayConnectionManager.cs` | The three connection-domain owners are authoritative. Keep only the public lifecycle façade, manager-owned operator/state/tunnel orchestration, typed source/sink/security ports, and event forwarding. |
